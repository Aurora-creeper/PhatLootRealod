...
import static com.codisimus.plugins.phatloots.PhatLoots.logger;
...
public class PhatLootChest {
    ...
    private static LinkedList<Block> findRedstone(Block block, boolean on) {
        ...
        EnumSet rSet;
        if(on){
            rSet = triggeredRedstone;
        }else{
            rSet = unTriggeredRedstone;
        }
        Block neighbor = block.getRelative(1, 0, 0);
        if (rSet.contains(neighbor.getType())) {
    ...
    private static void trigger(Block block) {
        ...
        case DROPPER: //Drop
            ...
            return;
        case PISTON: //Toggle the extended bit
            //TODO.
            if(((Piston)data).isExtended()){
                ...
                //player.playSound(block.getLocation(), Sound.BLOCK_PISTON_CONTRACT, 1.0f, 1.0f);
        ...
        case REPEATER:
            ...
            return;
        case REDSTONE_LAMP: //Turn the Lamp on or off
        case REDSTONE_TORCH:
